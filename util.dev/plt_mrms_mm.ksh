#!/bin/ksh 
set -x
# plot a 24h (12Z-12Z) total of MountainMapper MRMS
#
day=$1
...
# MRMS: MountainMapper_QPE_01H_00.00_20160602-170000.grib2
DCOM=/dcomdev/us007003/ldmdata/obs/upperair/mrms/conus/MountainMapper_QPE
...
do
  ...
  mrmsfile=MountainMapper_QPE_01H_00.00_${yyyymmdd}-${hh}0000.grib2
  cp $DCOM/$mrmsfile.gz .
  gunzip $mrmsfile
  err=$?
  if [ $err -ne 0 ]; then
...
cat > mrms_mm.ctl <<EOF1
dset ^MountainMapper_QPE_01H_00.00_%y4%m2%d2-%h20000.grib2
index ^mrms_mm.idx
options template
undef 9.999E+20
title MRMS
...
zdef 1 linear 1 1
vars 1
pcp   0,102,0   209,6,23 ** Precip Accum kg/m2
ENDVARS
EOF1

cat > pltgrads_mrms_mm.gs <<EOF2
'open mrms_mm.ctl'
...
'draw title MM MRMS 24h pcp ending 12Z ${day}'
'printim mrms_mm.${day}12.24h.gif gif x480 y425 white'
'quit'
EOF2

gribmap -i mrms_mm.ctl

grads -blc "run pltgrads_mrms_mm.gs"
exit
